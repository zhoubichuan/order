  ...
  <div class="cantai">
    ...
    <ul class="desktop_box">
      <li
        ...
        :class="item.deskStatus == 2 ? 'busy' : ''"
        @click="tableClick(key)"
      >
...
export default {
  ...
  methods: {
    ...
    searchTableListByAreaId(id) {
      ...
      this.axios.get("/order/static/table.json", params).then((resp) => {
        this.tableList = resp.data.result.deskTopList;
      });
    },
  ...
  created() {
    ...
    this.axios.get("/order/static/desk.json").then((resp) => {
      this.areaList = resp.data.result.deskAreaList;
      //默认第一个高亮
      this.changearea(0);
